...
| `audit.external-device-health` | `tracked-open` | External audit-device configuration and TCP reachability are reported as evidence only until an explicit write acknowledgement returns ready; unverified or unavailable external devices do not satisfy required audit policy while write-ack probe semantics remain tracked for AI review. | `crates/paranoid-audit/src/lib.rs`; `crates/paranoid-ops/src/lib.rs`; stable federal startup fixtures; CLI evidence tests; `docs/reference/ai-review.md` |
