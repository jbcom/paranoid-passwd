...
Those `262144`/`3`/`1` values are not just a fixed constant applied blindly: vault creation calls `paranoid_vault::calibrate_kdf_params` (`crates/paranoid-vault/src/kdf_calibration.rs`), which benchmarks Argon2id at a hard memory floor (`MEMORY_COST_FLOOR_KIB`, equal to `262144`) on the host and raises the iteration count toward a ~350ms interactive wall-clock target. Memory cost is only ever raised, never lowered, below that floor — a test (`calibration_never_emits_memory_below_floor`) proves the floor holds even when the benchmark suggests a much shorter target. If the floor-memory benchmark itself fails on a heavily constrained host, calibration falls back to the fixed `DEFAULT_*` constants (still at the floor) and reports that fallback rather than failing vault creation. Chosen parameters are persisted in `VaultHeader.kdf` exactly as calibration produced them, so this is a creation-time-only change with no unlock-path or wire-format impact.
