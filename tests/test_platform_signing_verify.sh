...
SMOKE_SCRIPT="${REPO_ROOT}/scripts/smoke_test_release_artifact.sh"
RELEASE_VALIDATE_SCRIPT="${REPO_ROOT}/scripts/release_validate.sh"
RELEASE_PATH_UTILS="${REPO_ROOT}/scripts/release_path_utils.sh"
PUBLISHED_RELEASE_SCRIPT="${REPO_ROOT}/scripts/verify_published_release.sh"
...
dupe_payload_root="${tmpdir}/dupe-payload"
...
mkdir -p "${dupe_payload_root}/one" "${dupe_payload_root}/two"
: > "${dupe_payload_root}/one/LICENSE"
: > "${dupe_payload_root}/two/LICENSE"
...
assert_ok "MSI WXS package code is generated by WiX" \
  grep -Fq 'Package Id="*"' "${BUILD_SCRIPT}"

assert_ok "release validation can defer signed Windows MSI verification to Windows hosts" \
  grep -q "signed Windows platform verification deferred to Windows host" "${RELEASE_VALIDATE_SCRIPT}"

assert_ok "published release verifier can require the Windows GUI MSI" \
  grep -q "PARANOID_REQUIRE_WINDOWS_MSI" "${PUBLISHED_RELEASE_SCRIPT}"

assert_ok "release download verification covers the Windows GUI MSI on Windows" \
  bash -c 'grep -q "paranoid-passwd-gui windows/amd64 msi" "$1" && grep -q "runner: windows-2022" "$1" && grep -q "asset_suffix: windows-amd64.msi" "$1"' \
    _ "${RELEASE_WORKFLOW}"

assert_ok "MSI payload validation requires unique payload matches" \
  grep -q "find_exactly_one_file_named" "${PAYLOAD_SCRIPT}"

assert_fails "release path helper rejects duplicate payload matches" \
  bash -c 'source "$1"; find_exactly_one_file_named "$2" "LICENSE" LICENSE' \
    _ "${RELEASE_PATH_UTILS}" "${dupe_payload_root}"

assert_ok "release scripts share Windows path utility" \
  bash -c 'grep -q "source scripts/release_path_utils.sh" "$1" && grep -q "source scripts/release_path_utils.sh" "$2" && grep -q "source scripts/release_path_utils.sh" "$3"' \
    _ "${BUILD_SCRIPT}" "${PAYLOAD_SCRIPT}" "${SMOKE_SCRIPT}"
