...
- [x] P8.V.10 Set up a GUI render-to-image path (SoftwareRenderer to PNG under xvfb, or equivalent) so the Slint window can actually be screenshotted and visually verified. Right now the GUI's visual identity is only verifiable by reading markup — the 'does it actually look designed' gate cannot be closed on the GUI without a rendered pixel to READ.
  VERIFIED: the render-to-image path (SLINT_BACKEND=software under Xvfb, driven by the P8.5 SCREEN_SEQUENCE Timer in crates/paranoid-gui/src/lib.rs) already existed from the P8.5 re-baseline commit but this item was never actually run to close it — ran `make test-gui-visual-regression-emulate` end-to-end (real Linux builder image, real Xvfb, real Slint software renderer): 16 real per-screen PNGs captured (trust-gate, verified, vault-list, add-item, item-detail, generate, ways-in, locked x real/decoy), all 1280x1024 non-blank renders, real-vs-decoy action-bar skeleton-geometry invariant passed. 4 baselines (generate/ways-in x real/decoy) re-captured with byte diffs confirmed via `compare -metric AE` to be confined entirely to the freshly-generated password/sha256 text (random per run) — zero pixel delta in layout/labels/buttons. This closes evidence.md Finding G0 ("the GUI e2e harness only ever captures one end-of-run screenshot... could only confirm the .slint MARKUP, never a real rendered GUI screenshot") with actual pixel evidence, not markup inspection.
...
## P8.V2 — Remaining visual-verify punch-list (opus re-verify round 2: matchesDesign=FALSE, 8 items — all copy/vocab, NO security bugs; the cleartext-password gap is FIXED). Same class: developer/engineer strings leaking into persona-facing surfaces. Each is a rewrite to brand.md voice; re-run opus visual-verify after.

- [ ] P8.V2.1 TUI S15 unlock screen leaks a developer changelog note ('Native unlock now works directly from the TUI; env-based CLI inputs remain valid too.'), the raw vault filesystem path ('Current path: /tmp/…/vault.sqlite'), and an inline hotkey wall ('Unlock mode: Recovery Secret (p/m/b/c or Left/Right)') into the persona-facing content pane — rewrite to brand.md §3d calm-unlock voice, drop the changelog note, path, and mode-hotkey hint.
- [ ] P8.V2.2 TUI Home status line leaks the env-var name: 'Unlocked via recovery secret via PARANOID_MASTER_PASSWORD, with device fallback if available.' — replace with the plain brand.md §3 status ('Vault open. N items.'); never surface PARANOID_MASTER_PASSWORD to the user.
- [ ] P8.V2.3 Standalone generator RESULT screen thrusts the full audit dump inline (six tabs Summary|Compliance|Entropy|Stats|Threats|Self-Audit, 'AUDIT PASSED', raw Hash=, char-mix counts, statistical verdicts) — ia.md §4 requires the math one drill-level down behind '⋯ Show the evidence'; lead with only the ✓ Randomness check verdict + ▸ Copy.
- [ ] P8.V2.4 GUI ways-in rows show raw internal enum ('password_recovery', 'mnemonic_recovery', 'mnemonic-2dfa955337b67538') — apply the same relationship-vocab mapping the TUI ways-in already uses (keyslot→way in, recovery keyslot→recovery phrase per brand.md §4).
- [ ] P8.V2.5 GUI generate shows 'sha256=… pass=true' inline in the RESULT panel despite a 'Show the evidence' disclosure existing — move the sha256/pass evidence behind the collapsed disclosure so ia.md §6 (collapsed-by-default) holds.
- [ ] P8.V2.6 TUI S10 Ways-in status line and detail-panel title still leak keyslot-enum vocab ('Keyslot view active… enroll a new mnemonic, device, or certificate slot', 'Keyslot Detail') even though the list rows were fixed; also the footer shows 'k mechanics' where ia.md §5 specifies 'a add'.
- [ ] P8.V2.7 S2d fingerprint drill body references the brand-demoted flag '--federal-evidence' — remove the government-branded flag name from the user-facing surface (brand.md §3c: it is 'evidence bundle').
- [ ] P8.V2.8 GUI vault-list row uses engineer pipe format leaking 'duplicates=0' and 'folder=' field names; and a test-automation status string ('GUI automation passed. Generator audit, typed ops policy events, vault CRUD…') is rendered in the GUI action bar on every screen — neither is a real persona-facing product string.
